...
def geoserver_webservice_create_role_api_action(context, data_dict=None):
    """
    ...
    if tk.c.userobj and tk.check_access('geoserver_role_modify', {'user':tk.c.userobj.name}):
        ...
        try:
            GeoserverRoleModel(user_id=user.id, role=role).save()
            ...
            raise tk.ValidationError("Bad request: Invalid request. {e}") 
    raise tk.NotAuthorized()

def geoserver_webservice_delete_role_api_action(context, data_dict=None):
    """
    ...
    if tk.c.userobj and tk.check_access('geoserver_role_modify', {'user':tk.c.userobj.name}):
        ...
        if user is None:
            raise tk.ValidationError(f"Bad request: Invalid request. User: {id} does not exist")
        user_roles = [x for x in GeoserverRoleModel.get_user_roles(user.id) if x.state == core.State.ACTIVE]
...
def geoserver_webservice_api_action(context, data_dict=None):
    ...
    if user is not None:
        user_roles = tk.get_action('get_geoserver_user_roles')({"user":user.id}, data_dict={"user_id":user.id})
        role_str = ', '.join(user_roles.get('roles'))
        return {
...
api_actions = {
    ...
    'create_geoserver_role': geoserver_webservice_create_role_api_action,
    'delete_geoserver_role': geoserver_webservice_delete_role_api_action,
